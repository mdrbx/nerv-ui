...
<Callout type="info" title="INTERNAL VISUAL FAMILY">
  The underlying rounded plate helper is internal on purpose. Public components expose this grammar through higher-level modules like <Link href="/docs/components/card">Card</Link>, <Link href="/docs/components/pilot-card">Pilot Card</Link>, <Link href="/docs/components/pattern-alert">Pattern Alert</Link>, and <Link href="/docs/components/status-stamp">Status Stamp</Link>.
</Callout>
...
- <Link href="/docs/components/card">Card</Link> can opt into this family with `rounded`; `variant="video"` is the orange footage palette, not the switch that enables the rounded plaque.

...
- Prefer <Link href="/docs/components/card">`<Card rounded />`</Link> for plaque geometry; add `variant="video"` when you specifically want the orange footage palette. <Link href="/docs/components/pilot-card">Pilot Card</Link>, <Link href="/docs/components/pattern-alert">Pattern Alert</Link>, and <Link href="/docs/components/status-stamp">Status Stamp</Link> remain the higher-level camera-overlay components.
...
<ComponentPreview title="CAMERA OVERLAY">
  <div className="relative overflow-hidden border border-nerv-mid-gray/50 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.1),transparent_35%),linear-gradient(180deg,#181818_0%,#020202_100%)] p-5">
    ...
    <div className="relative ml-auto max-w-sm">
      <Card
        eyebrow="TEST PLUG-02"
        title="KAWORU NAGISA"
        subtitle={"HARMONICS TEST PROCEEDING\nSUBJECT : FIFTH C."}
        variant="video"
        rounded
        footer="SIGNAL STATE : LIVE"
      />
    </div>
  </div>
</ComponentPreview>
